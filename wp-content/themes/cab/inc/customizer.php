...
function cab_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'cab_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'cab_customize_partial_blogdescription',
		) );
	}
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'cab_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'cab' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => cab_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'cab' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'cab' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'cab' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'cab' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'cab' );
}
add_action( 'customize_register', 'cab_customize_register', 11 );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since cab 1.5
 * @see cab_customize_register()
 *
 * @return void
 */
function cab_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function cab_customize_partial_blogdescription() {
	bloginfo( 'description' );
}

/**
 * Register color schemes for cab.
 *
 * Can be filtered with {@see 'cab_color_schemes'}.
 *
 ...
 */
function cab_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with cab.
	 *
	 ...
	 * }
	 */
	return apply_filters( 'cab_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'cab' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'cab_get_color_scheme' ) ) :
/**
 * Get the current cab color scheme.
 *
 * @since cab
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function cab_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = cab_get_color_schemes();
...
endif; // cab_get_color_scheme

if ( ! function_exists( 'cab_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for cab.
 *
 * @since cab
 *
 * @return array Array of color schemes.
 */
function cab_get_color_scheme_choices() {
	$color_schemes                = cab_get_color_schemes();
...
endif; // cab_get_color_scheme_choices

if ( ! function_exists( 'cab_sanitize_color_scheme' ) ) :
/**
 ...
 */
function cab_sanitize_color_scheme( $value ) {
	$color_schemes = cab_get_color_scheme_choices();
...
endif; // cab_sanitize_color_scheme
...
function cab_color_scheme_css() {
	...
	$color_textcolor_rgb         = cab_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = cab_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = cab_get_color_scheme_css( $colors );

	wp_add_inline_style( 'cab-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'cab_color_scheme_css' );
 ...
 * @since cab
 */
function cab_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', cab_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'cab_customize_control_js' );
 ...
 * @since cab
 */
function cab_customize_preview_js() {
	wp_enqueue_script( 'cab-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'cab_customize_preview_js' );
 ...
 */
function cab_get_color_scheme_css( $colors ) {
 ...
 * @since cab
 */
function cab_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-cab-color-scheme">
		<?php echo cab_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'cab_color_scheme_css_template' );
