...
if ( !defined( 'ABSPATH' ) ) { die; } # famous cheatin', huh?
...
class Plugin {
	...
	function join( $content, $css ) {
		return '<!-- ' . __( 'Added by Post/Page specific custom CSS plugin, thank you for using!', 'phylaxppsccss' ) . ' -->' . PHP_EOL .
		'<style type="text/css">' . $css . '</style>' . PHP_EOL .
		$content;
	}
	...
	function add_meta_boxes() {
		if ( current_user_can('manage_options') ) {
			add_meta_box(
				'phylax_ppsccss',
				__( 'Custom CSS', 'phylaxppsccss' ),
				[ $this, 'render_phylax_ppsccss' ],
				[ 'post', 'page' ],
				'advanced',
				'high'
        	);
	...
	function render_phylax_ppsccss( $post ) {
		...
		switch( $post->post_type ) {
			case 'post':	$screen = __( 'Post custom CSS', 'phylaxppsccss' ); break;
			case 'page':	$screen = __( 'Page custom CSS', 'phylaxppsccss' ); break;
		}
		if ( '' == $screen ) { return; }
...
?>
			...
			<p class="post-attributes-label-wrapper"><label
				for="phylax_ppsccss_single_only"><input
				type="hidden" name="phylax_ppsccss_single_only" value="0"><input
				type="checkbox" name="phylax_ppsccss_single_only" value="1" id="phylax_ppsccss_single_only"<?php echo $checked; ?>> <?php  echo __( 'Attach this CSS code only on single page view', 'phylaxppsccss' ); ?></label>
			</p>
			<p>
				<?php
